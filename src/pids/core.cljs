...
(rum/defc linesDisp < rum/reactive [atomAsVector]
  [:div {:style {:max-height "500px" :max-width "950px" :overflow "auto" :white-space "nowrap"}}
         (for [line (rum/react atomAsVector)]
           [:div [:span (str (:pid line) (if config/showms (" ms:" (:ms line))) (if config/showdisco (str " dc:" (:dc line))))]
